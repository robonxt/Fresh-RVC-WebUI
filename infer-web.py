...
with gr.Blocks(title="Fresh RVC WebUI") as app:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem(tab_modelInference):
                    ...
                    with gr.Column():
                        ...
                        with gr.Group() as tab1SingleIndex:
                            ...
                            file_index2 = gr.Dropdown(
                                label=i18n("Select Index (Dropdown)"),
                                choices=sorted(index_paths),
                                interactive=True,
                            )
                            ...
                            with gr.Accordion(
                                ...
                                gr.Markdown(i18n("自动检测index路径,下拉式选择(dropdown)"))
                                gr.Markdown(i18n("检索特征占比"))

                        # with gr.Group() as tab1SingleTest:
                        #     file_big_npy1 = gr.Textbox(
                        #         label=i18n("特征文件路径"),
                        #         placeholder="\"D:\\path\\to\\RVC\\logs\\TEST.npy\"",
                        #         interactive=True
                        #     )
                    ...
                    with gr.Column():
                        with gr.Group() as tab1SingleTransposing:
                            ...
                            with gr.Accordion(
                                ...
                                gr.Markdown(i18n("男转女推荐+12key, 女转男推荐-12key, 如果音域爆炸导致音色失真也可以自己调整到合适音域. "))

                        with gr.Group() as tab1SingleSampling:
                            resample_sr0 = gr.Slider(
                                minimum=0,
                                maximum=48000,
                                label=i18n("Bitrate Resampling"),
                                value=0,
                                step=1,
                                interactive=True,
                            )
                            rms_mix_rate0 = gr.Slider(
                                minimum=0,
                                maximum=1,
                                label=i18n("Voice Envelope Scaling"),
                                value=0.25,
                                interactive=True,
                            )
                            protect0 = gr.Slider(
                            ...
                            with gr.Accordion(
                                ...
                                gr.Markdown(i18n("输入源音量包络替换输出音量包络融合比例，越靠近1越使用输出包络"))
                                gr.Markdown(i18n("保护清辅音和呼吸声，防止电音撕裂等artifact，拉满0.5不开启，调低加大保护力度但可能降低索引效果"))
            ...
            with gr.Accordion(label=i18n("Batch File Conversion"), open=False):
                ...
                with gr.Row():
                    with gr.Column():
                        with gr.Group() as tab1BatchAudioInput:
                            dir_input = gr.Textbox(
                                label=i18n("输入待处理音频文件夹路径(去文件管理器地址栏拷就行了)"),
                                placeholder="\"D:\\path\\to\\RVC\\todo-songs\"",
                            )
                            with gr.Accordion(
                                label=i18n("Select by Files"),
                                open=False
                            ):
                                inputs = gr.File(
                                    file_count="multiple",
                                    label=i18n("也可批量拖拽音频文件, 二选一, 优先读文件夹，文件夹留空则读取拖拽文件")
                                )
                        ...
                        # with gr.Group() as tab1BatchTest:
                    ...
                    with gr.Column():
                        with gr.Group() as tab1BatchTransposing:
                        ...
                        with gr.Group() as tab1BatchSampling:
                    ...
                    with gr.Column():
                        with gr.Group() as tab1BatchF0Method:
                            f0method1 = gr.Radio(
                                label=i18n("Pitch Extraction Algorithm"),
                                choices=["pm", "harvest", "crepe", "rmvpe"]
                                if config.dml == False
                        ...
                        with gr.Group() as tab1BatchFormat:
                            format1 = gr.Radio(
                                label=i18n("导出文件格式"),
